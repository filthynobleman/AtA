...
int main(int argc, const char** argv){
	...
	//    fprintf(deb, "grid is %d x %d\n", nprow, npcol);
	    ...
	    MKL_INT np, mp, nb = 1, mb = 1;

	    np = numroc_( &n, &nb, &myrow, &zero, &nprow );
	    mp = numroc_( &m, &mb, &mycol, &zero, &npcol );

	    long long lldA = n;
	    long long lldC = m;
	    long long lldSubA = np;
	    long long lldSubC = mp;
	    MKL_INT info;
	    real dzero = 0;
	    real done = 1;
	    real *subA = (real*)calloc(np * mp, sizeof(real));
	    real *subC = (real*)malloc(mp * mp * sizeof(real));
		...
		descinit_(descSubC, &m, &m, &mb, &mb, &zero, &zero, &icontxt, &lldSubC, &info); 
